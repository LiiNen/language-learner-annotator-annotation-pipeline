...
def save_response(res_output_path, context_id, user_id, response, isPassed):
    if isPassed:
        file_path = '%s/%s__res__%s.json' % (res_output_path, context_id, user_id)
    else:
        file_path = '%s/no_pass__%s__res__%s.json' % (res_output_path, context_id, user_id)
    with open(file_path, 'w') as f:
